...
5. Upload the calibration PNGs privately for testing, including each `*-alpha.png` fixture, add the matching `ShirtGraphic`, `Shirt`, or `Pants` object to the rig, and assign its image property as Roblox documents.
...
regenerates the three orientation fixtures and `shirt-alpha.png`, `pants-alpha.png`, and `tshirt-alpha.png` deterministically from the registry in `src/domain/registry-data.ts` (script: `scripts/generate-calibration.mjs`, headless Chromium via the pinned Playwright package). The fixtures are committed artifacts and are regenerated only deliberately. The script prints each fixture's byte size and sha256 and self-verifies the PNG signature and IHDR dimensions before exiting non-zero on any mismatch.

## Evidence contract

Roblox's classic-clothing documentation currently establishes the supported garment types, PNG/JPEG input, official sizes, and Studio test procedure, but does not explicitly promise classic-clothing alpha behavior. The official 2026 shirt/pants template PNGs are RGBA and fully opaque. `SurfaceAppearance` alpha rules apply to PBR texture workflows and are not treated as proof for classic clothing here.

`npm run check:release` validates exactly the following. Every item must hold for the gate to pass.

1. The three `calibration/fixtures/*-alpha.png` files exist with canonical PNG dimensions. Their generator also samples the clear interior at alpha 0 and the exterior at alpha 255 before encoding.
2. `calibration/evidence/measurements.json` exists, parses as JSON, and satisfies:
...
3. Studio captures: `calibration/evidence/captures/{shirt,pants,tshirt}-studio-{front,back,left,right,top,bottom}.png` — all 18 files exist, are non-empty, and start with the PNG signature.
4. Web captures: the same 18 files with `-web-` in the name, captured from the web preview.
5. Completed checklist: `calibration/evidence/r6-checklist-completed.md` exists, contains `RESULT: PASS`, contains neither `RESULT: FAIL` nor `RESULT: PENDING`, and no table row has an empty `Result` cell.
6. Registry calibration: `src/domain/registry-data.ts` contains no occurrence of `calibrationVersion: null` (after calibration the three entries carry the recorded calibration version).

...
2. Add failing archive tests for:
   - opening a valid v1 ZIP and receiving a normalized v2 document;
   - saving and reopening a deterministic v2 ZIP;
...
7. Mechanically update compositor, preview, UI, export, and tests to use `ProjectDocument` while preserving behavior.
8. Run:
...
4. Extend the controller with a draw branch that takes precedence over selection/viewport gestures while the mode is active. Track one pointer, expose a draft rectangle callback, and convert a below-threshold drag to the default size. Clamp default size to a useful fraction of the current canonical canvas and keep its center within the canvas.
...
4. Preserve the canonical suffix order when building `layersTopFirst`; visually group/pin cutouts first. Hide cutout up/down buttons rather than showing no-op controls.
...
1. Add a browser project journey that draws a cutout, saves a `.rbxcloth.zip`, reopens it, and verifies the same selected geometry and alpha result.
2. Add a preview smoke assertion with a solid garment and a large cutout: the avatar body color must be visible through the cleared clothing region. Avoid screenshot-only assertions when a stable pixel probe is available.
3. Add E2E journeys for:
   ...
   - opening a legacy v1 fixture and resaving v2.
...
2. State that T-Shirt transparency only removes the front graphic, while Shirt/Pants alpha can reveal the avatar.
3. State that export is PNG, Studio testing is required, and uploading/moderation remains outside the app.
...
## Task 9: Independent implementation review and corrections
...
1. Run a local evidence audit against the approved design: trace schema, reducer invariants, compositor alpha, UI cancel paths, save/open, preview, export, offline behavior, accessibility, and mobile layout.
...
## Task 10: Full verification, push, deploy, and public smoke test
